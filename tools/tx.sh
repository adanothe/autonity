...
if [ "$currency" == "1" ]; then
    tx_hash=$(autonity tx make --to $chosen_address --value $value | autonity tx sign - | autonity tx send -)
elif [ "$currency" == "2" ]; then
    tx_hash=$(autonity tx make --to $chosen_address --value $value --ntn | autonity tx sign - | autonity tx send -)
fi
